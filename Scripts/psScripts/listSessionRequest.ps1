<#
    .SYNOPSIS
	    Lists basic details about session requests.
	.SYNTAX
	    listSessionRequest.ps1 [-status ALL|PENDING|ACTIVE|CURRENT|OPEN] [-requestorName <String>]
		[-systemName <String>] [-accountName <String>] [-MaxRows <int32>]
    .EXAMPLE
        listSessionRequest.ps1 -systemName dc1 -accountName testAcc -requestorName requestor
  #>
param(
...
try {
	...
	#endregion
	$requests = @()
	$res = $clnt.listSessionRequest($filter,[ref]$requests)
	if ($res.entries) {
	    ...
	    'listSessionRequest: ' + $res.message
	}
} catch {
