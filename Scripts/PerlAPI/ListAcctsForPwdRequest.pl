...
($rc, $count, $msg,  @list) = $client->listAcctsForPwdRequest(
...
if ($rc == 0)
       {
          ...
          {
		   @myNames = ('systemName', 'accountName', 'nowFl', 'details', 'reqID', 'maxReleaseDuration', 'requireTixForRequest', 'ticketSystem', 'lastReleaseDt', 'description', 'totalRows', 'acctLockFl', 'policyName', 'minApprovers');
			foreach (@myNames)
			{
			print " $_: $list[$i]{$_}\n ";
			} 
