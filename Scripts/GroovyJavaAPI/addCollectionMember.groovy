...
try {
	...
	CollectionMembership collmem = new CollectionMembership()
	
	collmem.setCollectionName(CollectionName)
	collmem.setSystemName(SystemName)
	collmem.setAccountName(AccountName)
	collmem.setFileName(FileName)

    CodeMessageResult res = new CodeMessageResult()
    acl.addCollectionMember(collmem, res)
