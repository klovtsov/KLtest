<#
    .SYNOPSIS
	    Lists specific details about password requests.
	.SYNTAX
	    listRequestDetails.ps1 [-status ALL|PENDING|ACTIVE|CURRENT|OPEN] [-requestorName <String>]
		[-systemName <String>] [-accountName <String>] [-startDate 'YYYY/MM/DD'] 
		[-endDate 'YYYY/MM/DD'] [-MaxRows <int32>]
    .EXAMPLE
        listRequestDetails.ps1 -systemName dc1 -accountName testAcc -startDate '2013/11/03'
  #>
param(
...
try {
	...
	#endregion
	$requests = @()
	$res = $clnt.listRequestDetails($filter,[ref]$requests)
	if ($res.entries) {
