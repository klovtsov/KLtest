...
try {
	...
	PwdRequest sr = new PwdRequest()

	if (!UserName.equals("")) {
//		println("UserName used!")
		sr.setForUserName(UserName)
	}
...
//		println("AccountName used!  ")
    ...
    sr.setRequestImmediateFl("N")
	sr.setRequestNotes("Blah blah blah!")
	
	Calendar rrd = Calendar.getInstance();

    rrd.set(Calendar.HOUR_OF_DAY, 3);
    rrd.set(Calendar.MINUTE, 0);
    rrd.add(Calendar.DATE, +5);
    sr.setReqReleaseDt(rrd);
			
    IDResult res2 = new IDResult()
    acl.addPwdRequest(sr, res2)
        
    GetFinalMessage = res2.getMessage()
