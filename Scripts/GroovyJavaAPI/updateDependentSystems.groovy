...
for (String s: args) {
	...
	} else if (paramName.equals("ADSystemName")) {
		ADSystemName = paramValue
	
		
	} else if (paramName.equals("ADAccountName")) {
		ADAccountName = paramValue				
...
try {
	...
	APICommandLib acl = new APICommandLib(scc)
     ...
     assign.add(SystemName)

    CodeMessageResult resCM = new CodeMessageResult()

       
    acl.updateDependentSystems(ADSystemName, ADAccountName, assign, null, resCM)
	   
    GetFinalMessage = resCM.getMessage();
